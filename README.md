...
Event Bubbling like a bubble rising from the bottom to the surface. it works: when an event (like a click) happens on an element, it doesn't just stay there. It "bubbles up" to its parent, then its grandparent, and so on.
